...
struct TokenList tokenizer(char* str){
  ...
  for (int i = 0; i < length; i++){
    char* temp;
    if ((str[i] == '&') || (str[i] ==  ';')){
      temp = (char*)malloc((i-prevPos+1)*sizeof(char));
	...
	idxArgList++;
      ...
      if (i == length-1){
	temp = (char*)malloc(sizeof(char)*(i-prevPos+1));
	memcpy(temp, &str[prevPos+1], i-prevPos-1);
	temp[i-prevPos] = '\0';
	prevPos = i;
...
void foreground(char** arg, int argsize){
  ...
  if (argsize == 1){
	...
	deletePid(last->data);
	sigprocmask(SIG_UNBLOCK, &blocked, NULL);
	printf("[%d]\tDone\t\t", last->key);
	printArg(last->cmd, last->argsize);
	printf("\n");
      }
      if (stopped == 1){
	sigprocmask(SIG_BLOCK, &blocked, NULL);
	deletePid(last->data);
	sigprocmask(SIG_UNBLOCK, &blocked, NULL);
	tcgetattr(STDIN_FILENO, &(last->term));
       
	printf("[%d]\tStopped\t\t", last->key);
	printArg(last->cmd, last->argsize);
	printf("\n");
	sigprocmask(SIG_BLOCK, &blocked, NULL);
	insertFirst(last->key, last->data, SUS, last->cmd, last->argsize, last->term);
	sigprocmask(SIG_UNBLOCK, &blocked, NULL);
      }
      tcsetattr(STDIN_FILENO, TCSANOW, &terminal);
